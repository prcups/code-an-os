...
#我们令 线性空间中低16M的页面页号同物理页号意义对应 将kernel放在线性空间的低64M中(0~0x3FFFFFF) 接下来64MB task1(0x4000000~0x7FFFFFF)  最后64MB放task2(0x8000000~0xbFFFFFF) 
#物理内存中 kernel StartFrom 0x4000(4号页)  task1 StartFrom 0x5000(5号页) 				     task2 StartFrom 0x6000(6号页)
#线性空间中 kernel StartFrom 0x0          
#           task1 StartFrom 0x4000000 线性页号0x4000(高10位0x10 低10位0x0)    用户栈所在区域(0x7FFF000~0x7FFFFFF) 线性页号0x7FFF(高10位0x1F 低10位0x3FF) 
#           task2 StartFrom 0x8000000 线页号0x8000(高10位 0x20 低10位0x0)     用户栈所在区域(0xbFFF000~0xbFFFFFF) 线性页号0xBFFF(高10位0x2F 低10位0x3FF)         
...
setup_paging:
    ...
    movl  $0x2007, pg_dir+0x1F*4	#页目录第31项 指向0x2000 task1页表的起始页号 7->(111) 用户态 内核态可访问 可读写 有效
    movl  $0x3007, pg_dir+0x2F*4	#页目录第47项 指向0x3000 task2页表的起始页号 7->(111) 用户态 内核态可访问 可读写 有效
    ...
    movl  $0x5007, pg_task1+0*4         #task1页表第0项 指向0x5000 代码/数据区 7->(111) 用户态 内核态可访问 可读写 有效 (同下)
    movl  $0x6007, pg_task2+0*4         #task2页表第1项 指向0x6000 代码/数据区
    movl  $0x5007, pg_task1+0x3FF*4	#task1页表第1023项 指向0x5000 栈区
    movl  $0x6007, pg_task2+0x3FF*4     #task2页表第1023项 指向0x6000 栈区
    #将Cr0最高位置1 开启分页模式
